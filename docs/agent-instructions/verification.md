...
- Pushing a release candidate to a `release/**` branch automatically starts `.github/workflows/release-gate.yml`; `workflow_dispatch` and `v*` tag compatibility remain available. The blocking release gate must finish green **before** any Play upload. Its first real GitHub run is still post-push evidence, so local YAML and test checks do not substitute for it.
