...
**Status as of 2026-08-26: Engineering READY · Play submission nearly ready · Production
publication GATED.** The code, tests and signed bundle are done. What remains is not
engineering work: back up the upload key, complete the Play Data Safety and Health Apps
declarations precisely, and resolve/sign the §44 regulatory assessment before sending a
production release for review.
...
| **OPEN** | Correct Play developer account type | Two separate questions, not one — do not resolve this from §44 alone. "Not a medical device" (§44) is this app's own regulatory conclusion; "is this a Google Play health app" is Google's own policy classification, resolved through the **Health Apps declaration** row below, which **every** published app completes, including ones declaring no health features (§150–151). Whether an Organization account is required (§152) follows from that declaration's outcome. Complete the declaration first, then record the account requirement it implies. |
...
| **DONE 2026-08-26** | Store assets | *(owner)* Feature graphic and real-device screenshots are done. `Logo.png` at the repository root remains the traced source of `ic_launcher_foreground.xml` for the 512×512 icon. |
...
| **OPEN** | Physical-device release QA | *(owner)* The app has since been run on a Samsung device with real-device screenshots captured — the general "never touched hardware" gap is closed. What remains open is narrower: confirm the **exact** release APK from commits `68c85a3`/`0b2312f` (this release's build) was what was installed and exercised, not an earlier build. If it wasn't, re-run the §8 checklist against this build before closing this row. |
