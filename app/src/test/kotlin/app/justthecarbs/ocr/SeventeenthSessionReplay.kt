...
/**
 * Replays the 29 physical captures of `docs/Scan Evidence 3rd testr` through the production path.
 *
 * Deliberately mirrors [SixteenthSessionReplay], [FifteenthSessionReplay] and
 * [FourteenthSessionReplay] rather than inventing a fourth harness: the corpora must be scored by
 * the same rules, or a change that improves one and damages another would be invisible.
 */
internal object SeventeenthSessionReplay {
    /** The crop the device actually confirmed, identical across all 29 bundles' `selection.txt`. */
    ...
    data class Result(
        val capture: SeventeenthSessionCorpus.Capture,
    ...
    fun replay(capture: SeventeenthSessionCorpus.Capture): Result {
        ...
        // observation — the rule the previous session's build established.
    ...
    fun replayAll() = SeventeenthSessionCorpus.captures.map(::replay)
